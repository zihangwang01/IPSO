...
def I4fit_func(x_train, x_test, y_train, y_test):  # 调用sklearn中分类器，并计算准确率
    ...
    if x_train.shape[0] != 0:  # 防止没有特征被选择
        model3 = XGBClassifier(n_estimators=21, max_depth=7, max_delta_step=3, learning_rate=0.0323,
...
def I4worst_update(X, size, wd, x_train, x_test, y_train, y_test , list1, t):
    ...
    # 选出最劣点
    fit=[]
    for i in range(size):    #计算每一个粒子的适应值大小
        fit.append(I4fitness_func(X.iloc[i],wd,x_train,x_test,y_train,y_test,t))
    a=fit.index(min(fit))    #选取出最差粒子
    X1.iloc[0] = copy.deepcopy(X.iloc[a])

    # 选出离最劣点最近的三个点
    juli=[]
    for j in range(X.shape[0]):
        juli.append(I4ed(X1.iloc[0,:],X.iloc[j,:],wd))
    tmp = zip(range(len(juli)), juli)
    large5 = heapq.nsmallest(4, tmp, key=lambda x: x[1])
    for i in range(1,4):
        # pdb.set_trace()
        b[i-1]=large5[i][0]   #获取距离最小的点的索引
        X1.iloc[i,:]=X.iloc[b[i-1],:]     #获取距离最小的点


    # 此时最差点是X1[a]，与该点距离最近的三个点为X1[b[0]],X1[b[1]]，X1[b[2]]
    ...
    fit1=[]
    fit1.append(fit[a])   #最差点的适应值
    for i in range(1,4):
        fit1.append(I4fitness_func(X2.iloc[i,:],wd,x_train,x_test,y_train,y_test,t))
    maxind=fit1.index(max(fit1))
    X.iloc[a,:]=copy.deepcopy(X2.iloc[maxind,:])
    return X
...
def I4pick(X, wd, size, gbest, t, x_train, x_test, y_train, y_test ):
    ...
    if t == 0:
        fit=[]
        for i in range(size):   #计算每一个粒子的适应值大小
            fit.append(I4fitness_func(X.iloc[i,:],wd, x_train, x_test, y_train, y_test , t)[0])
        tmp = zip(range(len(fit)), fit)    #选取出最大的四个元素
        large5 = heapq.nlargest(4, tmp, key=lambda x: x[1])
        ind=[]
        fitl=[]
        for i in range(len(large5)):   #获取最大的四个元素的索引位置
            ind.append(large5[i][0])
            fitl.append(fit[large5[i][0]])
        M=X.iloc[ind,:]
        M['fit']=fitl
        gbest.iloc[:4,:]=copy.deepcopy(M)
        return gbest
        ...
        M = X.iloc[ind, :]
        M['fit'] = fitl
        cdu = len(list(set(gbest)))
        if cdu != 8:
            for i in range(cdu, 8):
                gbest.loc[i] = [0] * (wd + 1)
        gbest.iloc[4:8,:]=copy.deepcopy(M)
...
def I4yind(gbest, y, wd, x_train, x_test, y_train, y_test , t):  # 计算每一个粒子的引导策略，其中gbest是最优的四个粒子,size是种群个数，wd是空间维度,y是受引导的粒子
    ...
    # pdb.set_trace()
    for i in range(0, 3):
        A[i] = I4My(I4ed(gbest.iloc[i], y, wd), gbest.loc[i,'fit'])
    m = 0
    for i in A:
        m += i
    Z = copy.deepcopy(y)
    ...
    for i in range(0, wd):  # 清除量纲的影响
        if Z[i] < 0.5:
            ...
            Z[i] = 1
    Y = (A[0] * (M.iloc[0, :wd] - Z) + A[1] * (M.iloc[1, :wd] - Z) + A[2] * (M.iloc[2,:wd] - Z)) / m
...
def I4velocity_update(X, pbest, gbest, t, size, max_val, wd, V, x_train, x_test, y_train, y_test ):
    ...
    for i in range(0, size):
        ...
        z = g * w * I4yind(gbest, X.iloc[i], wd, x_train, x_test, y_train, y_test, t).values
        # pdb.set_trace()
...
def I4pso(x_train, x_test, y_train, y_test , fit_list, list1):
    ...
    # 迭代计算
    for i in tqdm(range(0, iter_num)):
        t = t + 1
...
x_train = pd.read_csv(r'd:\new aco\信贷数据集\x_train.csv', encoding='gbk')
x_test = pd.read_csv(r'd:\new aco\信贷数据集\x_test.csv', encoding='gbk')
y_train = pd.read_csv(r'd:\new aco\信贷数据集\y_train.csv', encoding='gbk')
y_test = pd.read_csv(r'd:\new aco\信贷数据集\y_test.csv', encoding='gbk')
list2=x_train.columns.values.tolist()
list1 = ()
iter_num=100
for i in list2:
    list1+=(i,)
fit_list = np.zeros(shape=(20, 120))
