...
def PSfit_func(x_train, x_test, y_train, y_test):#调用sklearn中分类器，并计算准确率
    ...
    if x_train.shape[0] != 0:  # 防止没有特征被选择
        model1 = XGBClassifier(n_estimators=21, max_depth=7, max_delta_step=3, learning_rate=0.0323,
...
def PSpso(x_train, x_test, y_train, y_test,fit_list,list1):
    ...
    gbestL = np.zeros(shape=(iter_num,wd+1))  # 保留数据点的选择情况
    ...
    for time in range(0, iter_num):
        ...
        # pdb.set_trace()
        gbestL[time] = copy.deepcopy(gbest.iloc[0].tolist())
        for i in range(wd):
            if gbestL[time][i] >= 0.5:
                # pdb.set_trace()
                l1[i] += 1*gbestL[time][wd]
    ...
    for i in range(0,size):
        ...
        sc=plt.scatter(list,fit_list[i],c=list,label=1,vmin=0,vmax=120,cmap=cm)  #为第i个粒子绘制散点图
