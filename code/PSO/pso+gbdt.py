...
def PSfit_func(x_train, x_test, y_train, y_test):#调用sklearn中分类器，并计算准确率
    ...
    if x_train.shape[0] >= 10:  # 防止没有特征被选择
        model1 = GradientBoostingClassifier(learning_rate=0.123, n_estimators=41, max_depth=7,
                                          min_samples_leaf=60,
                                          max_features=9, subsample=0.7, random_state=1)
...
for i in list:
    list1+=(i,)
fit_list = np.zeros(shape=(20, 100))
