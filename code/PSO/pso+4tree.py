...
def PSfit_func(x_train, x_test, y_train, y_test):#调用sklearn中分类器，并计算准确率
    ...
    if x_train.shape[0] != 0:  # 防止没有特征被选择
        for i in range(0, 4):
            if i == 0:  # 调用GDBT
                gbm3 = GradientBoostingClassifier(learning_rate=0.053, n_estimators=43,
                ...
                model1 = cb.CatBoostClassifier(iterations=43, verbose=False,
                                               learning_rate=0.053, random_state=1)
                ...
                model2 = XGBClassifier(n_estimators=43,  learning_rate=0.053,
                ...
                model3 = LGBMClassifier(n_estimators=43, learning_rate=0.053,
                                        random_state=1)
